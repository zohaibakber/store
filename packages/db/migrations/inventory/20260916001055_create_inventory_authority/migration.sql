...
CREATE TABLE `consumed_tickets` (
	...
	CONSTRAINT `consumed_tickets_organization_id_nonce_hash_pk` PRIMARY KEY(`organizationId`, `nonceHash`)
);
--> statement-breakpoint
CREATE TABLE `download_leases` (
	...
	`pinnedHorizon` text NOT NULL,
	`expiresAt` integer NOT NULL,
	CONSTRAINT `download_leases_organization_id_replica_id_pk` PRIMARY KEY(`organizationId`, `replicaId`)
);
--> statement-breakpoint
...
CREATE TABLE `live_sessions` (
	...
	`deliveredThroughCommitSequence` text NOT NULL,
	`leaseExpiresAt` integer NOT NULL,
	CONSTRAINT `live_sessions_organization_id_session_id_pk` PRIMARY KEY(`organizationId`, `sessionId`)
);
--> statement-breakpoint
...
CREATE TABLE `replicas` (
	...
	`processedThroughClientSequence` text NOT NULL,
...
CREATE TABLE `snapshot_jobs` (
	...
	`startedAtCommitSequence` text NOT NULL,
	`horizon` text,
	`copyEntity` text,
	`copyCursor` text,
	`stepDueAt` integer NOT NULL,
	CONSTRAINT `snapshot_jobs_organization_id_snapshot_id_pk` PRIMARY KEY(`organizationId`, `snapshotId`)
);
--> statement-breakpoint
CREATE TABLE `snapshot_parts` (
	...
	CONSTRAINT `snapshot_parts_pk` PRIMARY KEY(`organizationId`, `snapshotId`, `partNumber`)
);
--> statement-breakpoint
CREATE TABLE `snapshot_staged_rows` (
	...
	`rowVersion` integer NOT NULL,
	`rowJson` text NOT NULL,
	CONSTRAINT `snapshot_staged_rows_pk` PRIMARY KEY(`organizationId`, `snapshotId`, `entity`, `entityId`)
);
--> statement-breakpoint
...
CREATE TABLE `wake_state` (
	`organizationId` text NOT NULL,
	`armedDueAt` integer NOT NULL,
	`reason` text NOT NULL,
	CONSTRAINT `wake_state_organization_id_pk` PRIMARY KEY(`organizationId`)
);
--> statement-breakpoint
...
CREATE INDEX `consumed_tickets_organization_id_expires_at_idx` ON `consumed_tickets` (`organizationId`,`expiresAt`);--> statement-breakpoint
CREATE INDEX `download_leases_organization_id_horizon_idx` ON `download_leases` (`organizationId`,`pinnedHorizon`);--> statement-breakpoint
...
CREATE INDEX `live_sessions_organization_id_replica_id_idx` ON `live_sessions` (`organizationId`,`replicaId`);--> statement-breakpoint
CREATE INDEX `live_sessions_organization_id_lease_idx` ON `live_sessions` (`organizationId`,`leaseExpiresAt`);--> statement-breakpoint
...
CREATE INDEX `snapshot_jobs_organization_id_stage_idx` ON `snapshot_jobs` (`organizationId`,`stage`);--> statement-breakpoint
