...
export const makeSyncSocketSession = Effect.fn("SyncSocketSession.make")(function* (input: {
  ...
  readonly httpExchange: (request: SyncRequest) => Effect.Effect<SyncResponse, SyncTransportError>;
  ...
  const exchange = Effect.fn("SyncSocketSession.exchange")(function* (request: SyncRequest) {
    ...
    if (!socket) return yield* input.httpExchange(request);
    ...
    const sent = yield* socket
      .send(JSON.stringify(encodeClientFrame({ type: "exchange", requestId, request })))
      .pipe(Effect.result);
    if (sent._tag === "Failure") {
      yield* dropPending(requestId);
      return yield* input.httpExchange(request);
    }
    return yield* Deferred.await(deferred).pipe(
      Effect.timeoutOrElse({
        duration: Duration.millis(input.exchangeTimeoutMillis ?? 45_000),
        orElse: () => dropPending(requestId).pipe(Effect.andThen(input.httpExchange(request))),
      }),
      Effect.catchIf(
        (error): error is SyncTransportError =>
          error instanceof SyncTransportError && error.retryable,
        () => input.httpExchange(request),
      ),
    );
  });
