...
- `client-db`. Shared inventory row models, PowerSync configuration, and
  authenticated Postgres mutation clients for the Electron renderer and Expo.
...
store. The Electron renderer uses WASQLite via `@powersync/web`. Expo uses
`expo-sqlite`. PowerSync fills the persisted collections from organization-scoped
Postgres streams.
