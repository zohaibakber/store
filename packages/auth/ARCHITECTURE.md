...
- The Clerk organization-binding D1 table is dropped. First-party
  `auth_user` / `auth_organization` / `auth_session` rows are the live identity
  store; no runtime path reads Clerk IDs.
