...
const packageManifests = [
  ...
  "scripts/migrate-cloudflare/package.json",
];

const deniedBuildEntry = /^ {2}"?([@\w./-]+)"?: false$/gmu;

const configTexts = [
  ["pnpm-workspace.yaml", read("pnpm-workspace.yaml").replace(deniedBuildEntry, "")],
...
requireText(read("packages/auth/src/http-api.ts"), '"/.well-known/jwks.json"', "auth JWKS route");
